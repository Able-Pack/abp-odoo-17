...
Select on Map is available alongside Google Maps Link in location lists, forms, and Attendance Settings. It uses the vendor's Leaflet 1.9.4/OpenStreetMap providers and requires internet access. Click or drag the pin, adjust Radius (meters), then Use This Location. The dialog updates the draft coordinates, radius, and Google Maps link together; Save the record/settings to persist. Cancel leaves the draft unchanged. All picker labels are English.
